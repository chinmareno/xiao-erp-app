...
export const POItemsInformationDetail = ({ loaderData }: Props) => {
  const POItemTotalParser = (itemPrice: number, itemQuantity: number) => {
  ...
  return (
    <div>
      ...
      <div className="border">
        {loaderData.POItems.map((POItem, index) => (
          <div
            key={index}
          ...
          >
            ...
            <div className="col-span-2">
              <p>{thousandSeparatorFormatter(String(POItem.quantity))}</p>
            </div>
            ...
            <div className="col-span-2">
              <p>
                {loaderData.priceCurrency === "IDR"
                  ? thousandSeparatorFormatter(String(POItem.costIdr))
                  : POItem.costYuan}
              </p>
            </div>
            <div className="col-span-2 mr-3 text-right self-center">
              <p>
                {" "}
                {loaderData.priceCurrency === "IDR"
                  ? POItemTotalParser(POItem.costIdr, POItem.quantity)
                  : POItemTotalParser(POItem.costYuan, POItem.quantity)}
              </p>
            </div>
          </div>
        ))}
