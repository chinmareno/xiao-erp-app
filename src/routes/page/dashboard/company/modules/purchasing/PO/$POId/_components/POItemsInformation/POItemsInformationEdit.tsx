...
export const POItemsInformationEdit = ({
  POItems,
  setPOItems,
  selectedSupplierId,
  params,
  ...
  const POItemTotalParser = (POItemPrice: string, POItemQuantity: string) => {
    if (!POItemPrice || !POItemQuantity) return "0";
    const isIDR = priceCurrency === "IDR";

    const price = isIDR
      ? Number(POItemPrice.replace(/,/g, ""))
      : parseFloat(POItemPrice);

    const quantity = Number(POItemQuantity.replace(/,/g, ""));
        ...
        {POItems.map((POItem, index) => (
          ...
          >
            ...
            <div className="col-span-2 mr-3 text-right self-center">
              <p> {POItemTotalParser(POItem.price, POItem.quantity)}</p>
            </div>
